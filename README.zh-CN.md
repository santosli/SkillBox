...
当前版本：`v0.9.3`。SkillBox 现在已经可以用于本地 skill 管理，但仍是早期软件。重要 skills 请保留备份，并在应用每一次文件系统变更前先 review。GitHub 多 skill collection preview/apply 首次随 v0.9.0 发布；collection 级更新/回滚仍计划在后续 v0.9.x 实现。
...
```text
SkillBox_0.9.3_universal.dmg
```

对应 checksum：

```text
SkillBox_0.9.3_universal.dmg.sha256
```
