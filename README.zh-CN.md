...
- **一个 managed store，面向多个 runtime。** SkillBox 把持久状态放在 `~/.skillbox`，再部署到 agent 目录，而不是让 runtime 目录成为唯一副本。
- **导入前先审查。** 本地扫描会先生成 import candidates，包括 user/remote 分类建议、system skill 处理、冲突和调用次数。
- **默认用 symlink 部署。** Runtime target 指回 managed store，用户 skill 编辑和远程版本回滚都不需要反复复制目录。
- **Rust 驱动核心 workflow。** 桌面命令调用 Rust crates 完成扫描、导入、部署、GitHub URL 解析、user skill Git 同步、remote source 绑定、更新检查、更新/回滚预览、workspace registry、usage hooks 和 operation history。
- **避免静默改写。** 既有的非 symlink runtime 内容不会被静默覆盖；导入流程会先复制到 managed store，并走带备份意识的替换路径。
