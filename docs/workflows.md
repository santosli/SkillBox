...
- 每张 collection card 的 header 统一提供 `Import as` User/Remote radiogroup。选择后会对完整 collection（不受搜索、Tab 或折叠状态影响）中所有 importable、无冲突且 selected variant 匹配的 pending child 设置同一类型并选中；用户随后仍可逐项取消。imported、system、conflict、invalid 和 read-only child 不会被重分类。child row 不再重复类型控件，只保留必要的只读状态说明；standalone group 仍使用自身的 User/Remote control。
