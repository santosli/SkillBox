...
- Added daily macOS updater metadata checks with a SQLite-backed successful-result cache, a sidebar Update reminder, a confirmation dialog before signed install/restart, and retry-safe pending updates without automatic downloads.
...
- Implemented evidence-aware local Skill Usage across Rust core, CLI, Tauri, and desktop. The desktop page is named Usage. schema v7 persists `confirmed/inferred/reference` plus bounded provenance, upgrades duplicate events only toward stronger evidence, and idempotently rebuilds `skill_usage_stats` from `confirmed + inferred` Calls without requiring a rescan. Codex structured per-turn skill carriers and dedicated `SKILL.md` file reads are inferred Calls; Cursor transcript assistant `Read`/`ReadFile` and user `manually_attached_skills` attachments are inferred Calls, deduplicated once per transcript user turn and skill, and can retain safe historical-missing lexical evidence without granting filesystem authority. Stop hooks and Claude Code native Skill attribution are confirmed, and Claude Code skill resolution can use other local runtimes or the managed store. Cursor state attachments and public `usage-record` events are references. Dashboard, Workspaces, History, Skill Detail, and Usage keep Calls separate from History references and count Calls from the same `skill_usage_events` `confirmed + inferred` aggregation. Ranking queries also return a local-calendar `daily` Calls series covering the last 365 days; the desktop Usage page charts it as a compact year heatmap and updates the Usage 统计 table when a day is selected. Coverage returns evidence totals, time ranges, overlapping provenance source counts, scan/backfill aggregates, and an aggregate-only `usage-audit`; it does not expose prompt/chat/tool bodies. Codex provider-native run totals remain unavailable, so Calls are explicitly a known local undercount. `Sync histories` is user-triggered, idempotent, and can recover or upgrade evidence while preserving successful provider results when another provider fails. The desktop scan overlay reports the current provider and processed/total files.
...
- [Git-backed Skill Collections](https://github.com/santosli/SkillBox/issues/46)
  are the active v0.9.x hardening track. Phase A+B shipped in v0.8.0 and Phase
  C shipped in v0.9.0: local repository grouping, schema-backed
  collection/member relationships, and one-fetch GitHub multi-skill
  preview/apply are available in Import Review. A collection records canonical
